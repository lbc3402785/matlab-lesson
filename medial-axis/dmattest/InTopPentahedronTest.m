...
p=[2,0.5,0];
% p=slab.V1_UP;
% p=(slab.V1_UP+slab.V2_UP)*0.5;
% p=(slab.V1_UP+slab.V2_UP+c1+c2)*0.25;
% n12=cross(c2-c1,slab.n0);
% p=p-1e-6*n12;
% p=(slab.V1_UP+slab.V2_UP+slab.V3_UP)/3-1e-6*slab.n0;
% p=c1;
% p=(c1+c2+c3)/3.0+1e-6*slab.tri.normal;
% p=slab.V1_DOWN;
plot3(p(1), p(2), p(3), 'o', 'MarkerSize', 10, 'MarkerEdgeColor', 'b', 'MarkerFaceColor', 'r');
[success]=slab.InTopPentahedron(p);
if success
