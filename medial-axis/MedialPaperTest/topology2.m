...
for i = 1:length(tets)
    ...
    for j = 1:size(vertices, 1)
        if ~isequal(vertices(j, :), B)  % 排除 O2 点
            plot3([A(1), vertices(j, 1)], [A(2), vertices(j, 2)], [A(3), vertices(j, 3)], 'k-', 'LineWidth', 1.5); % 实线
        end
    end

    % 绘制从 O2 到各点 Ai 的边（虚线）
    for j = 1:size(vertices, 1)
        if ~isequal(vertices(j, :), A)  % 排除 O1 点
            plot3([B(1), vertices(j, 1)], [B(2), vertices(j, 2)], [B(3), vertices(j, 3)], 'k--', 'LineWidth', 1.5); % 虚线
        end
    end
end

% 绘制外心
plot3(centers(:, 1), centers(:, 2), centers(:, 3), 'ko', 'MarkerSize', 6, 'MarkerFaceColor', 'k');
