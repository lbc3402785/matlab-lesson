...
%最后更新日期：
    ...
    if ( (dr0r1 < 1e-8) && (dr0r2 < 1e-8) && (dr1r2 < 1e-8) )
        ...
        if success
            ...
            %-----------------------------------------
            st0=Triangle(c0 + r0*newnorm0,c1 + r1*newnorm0,c2 + r2*newnorm0);
            if (dot(st0.normal,newnorm0)<0)
                st0=Triangle(c0 + r0*newnorm0,c2 + r2*newnorm0,c1 + r1*newnorm0);
            end
            n0=newnorm0;
            %-----------------------------------------
            st1=Triangle(c0 + r0*newnorm1,c1 + r1*newnorm1,c2 + r2*newnorm1);
            if (dot(st1.normal,newnorm1)<0)
                st1=Triangle(c0 + r0*newnorm1,c2 + r2*newnorm1,c1 + r1*newnorm1);
            end
            n1=newnorm1;
        end
    end
