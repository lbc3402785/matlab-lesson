...
%% Circumcenters(Nested)
    function [cc,r]=CC()
        ...
        Det=v41(:,1).*det23+v41(:,2).*det13+v41(:,3).*det12;%a
...
%% Connectivity(Nested)
    ...
    function [t2tetr,tetr2t]=Connectivity(tetr)
        ...
        numt = size(tetr,1);
        vect = 1:numt;
        t = [tetr(:,[1,2,3]); tetr(:,[2,3,4]); tetr(:,[1,3,4]);tetr(:,[1,2,4])];%triangles not unique [face_i0;face_i1;face_i2;face_i3;]
        [t,j,j] = unique(sort(t,2),'rows');%triangles,j=IC,t=t_unique(IC)
        ...
        nume = size(t,1);
        tetr2t  = zeros(nume,2,'int32');%每个三角形最多和两个四面体关联
        count= ones(nume,1,'int8');
        for k = 1:numt

            for j=1:4
                ce = t2tetr(k,j);
                tetr2t(ce,count(ce)) = k;%四面体关联
                count(ce)=count(ce)+1;
                if count(ce)>=3;
                    fprintf('%d tetrahedron incident to the same face\n',count(ce));
                end
                %每个三角形最多和两个四面体关联
            end
...
%% Walking(Nested)
    function t=Walking()
        np=size(p,1)-600;%540 = number of shield points put at the end of array
        numtetr=size(tetr,1);
        nt=size(tetr2t,1);
        ...
        for i=1:numtetr
            for j=1:4
                if tetr(i,j)>np;
                    deleted(i)=true;%外部单元
                    checked(i)=true;%己检查
                    onfront(t2tetr(i,:))=true;%t2tetr 每个四面体关联的三角形唯一编号,冗余四面体关联的唯一三角形
                    countchecked=countchecked+1;%检查数量
                    break;%有一点是边界点不用继续检查
                end
            end
        end
        ...
        for i=1:nt
            if tetr2t(i,2)>0 %jump boundary tetraedrom
                    ...
                    %d_{pow}(B_1,B_2)=d^2(c_1,c_2)-r_1^2-r_2^2;
                    %\cos(\alpha)=d_{pow}(B_1,B_2)/(2*r_1*r_2)
            end
        end
        clear cc
...
%         
        ...
        while countchecked<numtetr && level<50
            ...
            for id=1:nt%loop trough triangles
                %onfront：冗余四面体或者已经检查过的四面体关联的唯一三角形为true,其他三角形为false
                if onfront(id)
                    %tetr2t 唯一三角关联两个四面体，边界三角形只关联一个四面体
                    ...
                    if  tetr2==0 %do not check boundary triangles
                        onfront(id)=false;%不是物体表面三角形
                        continue
                       
                    elseif (checked(tetr1) && checked(tetr2)) %tetraedroms are already checked
                        %两个冗余四面体关联的唯一三角形不是物体的表面，或者这一对已经检查过了
                         onfront(id)=false;
                        continue
                       
                    end
                    %t2tetr 每个四面体关联的三角形唯一编号
                    if alpha(id)<=-toll(id) %flag as equal \alpha>arccos(-toll(id))深相交
                        %一个冗余四面体和非冗余四面体的关联三角形
                        if checked(tetr1)%find the checked one between the two tetr1冗余四面体
                            deleted(tetr2)=deleted(tetr1) ;%flag as equal
                            checked(tetr2)=true;%check 标记过的
                            countchecked=countchecked+1;
                            onfront(t2tetr(tetr2,:))=true;%put on front all tetreadrom triangles
                            %标记为已经检查过了的四面体关联的三角形防止重复计算
                        else%tetr1非冗余四面体
                    ...
                    elseif alpha(id)>toll(id)%flag as different \alpha<arccos(toll(id))浅相交
                        ...
                        %初始的迭代选择比较显著的内外极球相交的三角面，后面的迭代要求的alpha角允许大一点
                    end
            ...
            if level==31 %brute continuation(this may appens when the triangulation is corrupt)
                ...
                %put on front all tetreadrom triangles of unchecked tetrahedrons
            end
        end
        
        %delete tetraedroms marked as outside 
        tetr(deleted,:)=[];%清除外部cell
